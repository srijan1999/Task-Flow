    ...
    <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-50 dark:bg-slate-950 select-none transition-colors duration-200">
      ...
      <div className="space-y-3">
        {activities.map((act) => (
          <div 
            key={act.id} 
            className="flex gap-3 items-start p-3.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all"
          >
            <img 
              src={act.userAvatar} 
              alt={act.userName} 
        ...
        {activities.length === 0 && (
          <div className="text-center py-16 space-y-2">
            <Sparkles className="h-8 w-8 text-slate-300 dark:text-slate-700 mx-auto" />
            <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">No activity yet. Try moving a task or adding a comment!</p>
          </div>
        )}
      </div>
    </div>
