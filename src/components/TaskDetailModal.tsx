...
export const TaskDetailModal: React.FC<TaskDetailModalProps> = ({
  ...
  const handleSave = () => {
    onUpdateTask({ ...task, title, description, priority, status, assigneeId, dueDate, tagIds: selectedTagIds });
  };
  ...
  const handleToggleTag = (tagId: string) => {
    ...
    onUpdateTask({ ...task, tagIds: updatedTagIds });
  };

  const handleAddSubtask = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newSubtask.trim()) return;
    const sub: SubTask = { id: `sub-${Date.now()}`, title: newSubtask, completed: false };
    onUpdateTask({ ...task, subtasks: [...task.subtasks, sub] });
    setNewSubtask("");
  };
  ...
  const handleAddComment = (e: React.FormEvent) => {
    ...
    onUpdateTask({ ...task, comments: [comment, ...task.comments] });
    setNewComment("");
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-lg shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col max-h-[90vh] overflow-hidden relative z-10 transition-colors duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs font-bold ${accent.text} ${accent.textDark} ${accent.bgLight} ${accent.bgLightDark} px-2 py-0.5 rounded-full`}
            >
              ID: {task.id}
            </span>
          </div>
          <div className="flex items-center gap-1">
            <Button
              ...
              size="icon"
              onClick={() => {
                if (confirm("Are you sure you want to delete this task?")) onDeleteTask(task.id);
              }}
          ...
          <div className="space-y-2.5">
            ...
            <div className="space-y-1.5 max-h-[150px] overflow-y-auto pr-1">
              {task.subtasks.map((sub) => (
                <div
                  key={sub.id}
                  className="flex items-center justify-between bg-slate-50 dark:bg-slate-950/30 hover:bg-slate-50 dark:hover:bg-slate-950/80 p-2 rounded-xl border border-slate-100 dark:border-slate-800 transition-colors"
                >
